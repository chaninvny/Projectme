...
(function () {
  ...
  // opts.silent=true → don't show consent UI (used on page reload to refresh token)
  async function connect(clientId, opts = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // silent: try to get token without showing consent (works if user already granted)
      // non-silent (first time): show consent screen
      tokenClient.requestAccessToken({ prompt: opts.silent ? '' : 'consent' });
    });
  }

  // Silent reconnect on page load — best-effort, returns true on success
  async function silentReconnect(clientId) {
    try {
      await connect(clientId, { silent: true });
      return true;
    } catch (e) {
  ...
  window.LedgerCal = {
    connect, silentReconnect, disconnect, isConnected, getAccessToken,
